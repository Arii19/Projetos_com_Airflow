...
from grafana_loki_operator import GrafanaLokiOperator as loki
...
def DadosMeteorologicosOpenMeteo():
    ...
    def extract():
        try:
            ...
            loki.SendLog('dag=extracao_dados_climaticos_open_meteo def=extract log_level=ERRO message="'+str(e)+'"')
            raise
    ...
    def transform(df):
        try:
            ...
            loki.SendLog('dag=extracao_dados_climaticos_open_meteo def=transform log_level=ERRO message="'+str(e)+'"')
            raise
    ...
    def load(df):
        try:
            ...
            for row in df.itertuples(index=False, name=None):
                cursor.execute(
                    "EXEC sp_Merge_DadosClimaticos_OpenMeteo %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s",
                    row
                )
            conn.commit()
            cursor.close()
